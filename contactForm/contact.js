...
function messageArea(selector, uploadurl, i18n, configuration) {
	...
	var W = window,
		N = W.navigator,
		D = W.document,
		AC = W.AudioContext || W.webkitAudioContext,
		FR = W.FileReader,
		PE = W.PointerEvent,
		T8 = {
			speak: "speak",
			...
			recording: "recording...",
			playback: "Click to preview record.",
			cancel: "cancel",
			ok: "ok",
			record: "Click the microphone to record",
			drag: "Select or drag a file here",
			confirm: "confirm",
			ok: "ok",
			select: "select file",
			audio: "%n has recorded a message.",
			file: "%n has attached a file.",
			saved: "saved",
			error: "error",
			errorType: "file type not allowed.",
			errorSize: "%n is larger than %s.",
			delete: "delete",
			noSupport: "Audio API not supported.",
			noPermission: "No access to microphone.",
			rename: "rename",
			add: "add",
			confirmDelete: "Delete this item permanently?",
			noFile: "No file recognized.",
		},
		PTS = {
			pause: "M2 7h2V2H2v5zm3-5v5h2V2H5z",
			arrow: "M1 4l.8.7L4 2.4v6.1h1V2.4l2.2 2.3L8 4 4.5.5 1 4z",
			ok: "M3 8L.5 5.4l.7-.8L3 6.5 7.8 1l.7.8z",
			stop: "M2 2h5v5H2z",
			play: "M3 2v5l4-2.5z",
			no: "M1 1.7l2.8 2.8L1 7.3l.7.7 2.8-2.8L7.3 8l.7-.7-2.8-2.8L8 1.7 7.3 1 4.5 3.8 1.7 1l-.7.7z",
			rename:
				"M5,2H0.5v5H5V2z M7,2v5h1.5V2H7z M6.3,1h1V0.5H4.8V1h1v7h-1v0.5h2.5V8h-1V1z",
			trash:
				"M6.5 8.5h-4l-.5-6h5l-.5 6zM4 8h2l.4-5H4v5zM7.5 2h-6V1H3V.5h3V1h1.5z",
			add: "M4 .5V4H.5v1H4v3.5h1V5h3.5V4H5V.5H4z",
			...
			preview:
				"M6 .5v2h2l-2-2zM3.1 5.1c0 .5.4 1 1 1s1-.4 1-1-.4-1-1-1-1 .4-1 1zM5.5.5H1v8h7V3H5.5V.5zm.2 4.6l-.3.9 1 1.1-.5.4L5 6.4a3 3 0 01-.9.2c-.9 0-1.5-.7-1.5-1.5 0-.9.7-1.5 1.5-1.5s1.6.6 1.6 1.5z",
		...
		ACC = "audio/*,video/*,image/*,.pdf,.txt,.rtf,.doc,.docx",
		REC = false,
	...
	function Area() {
		...
		var t8 = wrap.dataset ? merge(T8, wrap.dataset) : T8,
			...
			sBtn = AC
				? createEl(
						"button",
						{ type: "button", "aria-label": t8.speak },
						null,
						svg(PTS.mic),
						nav
				  )
			...
			uBtn = createEl(
				"button",
				{ type: "button", "aria-label": t8.upload },
				null,
				svg(PTS.up),
				nav
			),
			...
			back = createEl(
				"button",
				{ type: "button", class: "back", "aria-label": t8.close },
				null,
				svg(PTS.arrow)
			),
			output = createEl(
				"fieldset",
				{ id: id + "-uploads", class: CLN + "-output" },
				null
			),
			browse = createEl(
				"input",
				{ type: "file", id: id + "-files", accept: ACC },
				null,
				null,
				nav
			),
		...
		function select(e) {
			...
			if (this === tBtn) {
				...
				if (areaC) {
					area.value = areaC;
					area.removeAttribute("data-cache");
				}
				nav.className += " type";
				...
				getMedia(
					...
					function (stream) {
						var ps =
							'<path d="' + PTS.play + '"/><path d="' + PTS.pause + '"/>';
						nav.className += " speak";
					...
					function (err) {
						Dialog("alert", err.name ? err.message : t8.noPermission);
					}
				);
			} else if (this === uBtn) {
		...
		function deselect(e) {
			...
			if (lablC) {
				label.textContent = lablC;
				label.removeAttribute("data-cache");
			}
			if (area.value.length) {
				area.setAttribute("data-cache", area.value);
				area.value = "";
			}
			nav.className = "select";
			area.setAttribute("readonly", true);
		...
		function handle(e) {
			...
			if ((typM || extM) && file.size <= max) {
				...
				r.onload = function (e) {
					Listing(file, r.result, "upload", ex);
				};
		...
		function Listing(file, data, type, ext) {
			...
			if (ext.match(/(svg|png|jpg|jpeg|gif|bmp|webp)/gi)) {
				var im = createEl("img", { src: data }, null);
				a = createEl(
					"a",
					{ href: data, title: file.name, target: "_blank", rel: "noopener" },
					txt,
					im,
					prt
				);
			} else if (ext.match(/(mp3|wav|ogg|weba)/gi)) {
				a = createEl(
					"a",
					{ href: "#", title: file.name, rel: "noopener" },
					txt,
					svg(PTS.mic),
					prt
				);
				...
				a = createEl(
					"a",
					{ href: data, title: file.name, rel: "download noopener" },
					txt,
					svg(PTS.up),
					prt
				);
			...
			ren = createEl(
				"button",
				{ type: "button", class: "rename" },
				t8.rename,
				svg([PTS.rename, PTS.no]),
				prt
			);
			upl = createEl(
				"button",
				{ type: "button", class: "upload" },
				t8.upload,
				svg([PTS.arrow, PTS.ok]),
				prt
			);
			del = createEl(
				"button",
				...
				upl.onclick = function () {
					...
					var int = setInterval(function () {
						if (i <= 100) {
							...
							clearInterval(int);
							prt.setAttribute("data-state", "sent");
							upl.onclick = upload;
						}
					}, 500);
		...
		function Recorder(stream, config) {
			...
			if (worklet) {
				...
				if (worklet) {
					...
					context.audioWorklet
						.addModule("input-processor-worklet.js")
						.then(function () {
							node = new AudioWorkletNode(context, "input-processor");
						});
				}
			...
			if (encoder) {
				encoder = URL.createObjectURL(
					new Blob(["(", encoder, ")();"], { type: "application/javascript" })
				);
			...
			encoder.postMessage({
				cmd: "init",
				config: { sampleRate: context.sampleRate, bitRate: 16 },
			});
			...
			function exit(e) {
				encoder.terminate();
				context.close();
				[].forEach.call(W.localStream.getTracks(), function (t) {
					t.stop();
				});
			...
			function playback(e) {
				...
				audio.ontimeupdate = function (e) {
					var time =
						formatMs(this.currentTime) + " / " + formatMs(this.duration || 0);
		...
		function Dialog(type, str, callback, persist) {
			...
			dialog = createEl(
				"form",
				{ role: "document", "aria-live": "polite" },
				"<i>" + message + "</i>" + buttons
			);
			modal = createEl(
				"div",
				{ class: CLN + "-dialog", role: "dialog", open: "" },
				null,
				dialog,
				wrap
			);
	...
	function merge(o1, o2) {
		return [o1, o2].reduce(function (a, o) {
	...
	function blobToDataURL(blob, res) {
		var r = new FR();
		r.onload = function () {
			return res instanceof Function ? res(r.result) : r.result;
		};
	...
	function trap(e) {
		...
		var els = this.querySelectorAll(
			"a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input:not([disabled]):not([hidden]):not([type=hidden]), select:not([disabled]):not([hidden])"
		);
	...
	function getMedia(constraints, res, err) {
		// Shim old getUserMedia API
		...
		if (N.mediaDevices.getUserMedia === undefined) {
			N.mediaDevices.getUserMedia = function (constraints) {
				...
				return new Promise(function (res, err) {
					gum.call(N, constraints, res, err);
				});
			};
